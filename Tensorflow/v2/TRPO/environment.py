...
class Environment(object):
    def __init__(self, env_name, params, logger):
        ...
        ns = NetworkStats(params.path, logger)
        self.MaxDegree = ns.CalculateStats2(env_name, adjacent)
        self.state_size = len(adjacent)
        self.action_space = self.state_size
        self.currentState = np.zeros(self.state_size, dtype=np.int16)
        self.logger = logger
    ...
    def checkActionDuplication(self, action):
        legal = True
        reward = 0
        for node in action:
    ...
    def checkActionCollision(self, action, count):
        legal = True
        reward = 0
        for i in range(count):
            node1 = action[i]
            if node1 > (self.state_size - 1) :
                legal = False
                reward = -1
                return legal, reward
            if self.currentState[node1] > 0:
                legal = False
                reward = -1
                return legal, reward
            for j in range(i + 1, count):
                ...
                if self.adjacent[node1][node2] > 0 or self.currentState[node1] > 0 or self.currentState[node2] > 0:
                    #print("self.adjacent[node1][node2]", self.adjacent[node1][node2],
                          #"self.currentState[node1]", self.currentState[node1],
                          #"self.currentState[node2]", self.currentState[node2])
                    legal = False
                    reward = -1
                    return legal, reward
                else:
    ...
    def checkActionLegal(self, action, count):
        #print("len(action)", len(action), "count", count)
        if count > 0:
            legal, reward = self.checkActionCollision(action, count)
        else:
            legal = False
            reward = -1
        #if legal:
            #legal, reward = self.checkActionDuplication(action)
        if not legal and reward >=0:
            reward = -1

        #print("legal ", legal, "len(action) ", len(action), "action ", action
    ...
    def step(self,action):
        #nodeList, count = self.convertAction(action)
        nodeList = action
        count = len(action)
        #print("count", count, "nodeList", nodeList)
        legal, reward = self.checkActionLegal(nodeList, count)
        ...
        #info = {}
        rewardarray = np.zeros(1, dtype=np.int16)
        rewardarray[0] = reward
        return next_state, rewardarray, done, legal
