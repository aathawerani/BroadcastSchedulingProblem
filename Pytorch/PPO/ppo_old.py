...
class Buffer:
    ...
    def __init__(self, observation_dimensions, size, gamma=0.99, lam=0.95):
        ...
        self.observation_buffer = np.zeros(
            ...
            #(size, 1, observation_dimensions), dtype=np.float32
            #(size, observation_dimensions, observation_dimensions), dtype=np.float32
            (size, observation_dimensions, observation_dimensions), dtype=np.short
        )
        #self.action_buffer = np.zeros(size, dtype=np.int32)
        self.action_buffer = np.zeros(size, dtype=np.short)
        #self.advantage_buffer = np.zeros(size, dtype=np.float32)
        self.advantage_buffer = np.zeros(size, dtype=np.short)
        #self.reward_buffer = np.zeros(size, dtype=np.float32)
        self.reward_buffer = np.zeros(size, dtype=np.short)
        #self.return_buffer = np.zeros(size, dtype=np.float32)
        self.return_buffer = np.zeros(size, dtype=np.short)
        #self.value_buffer = np.zeros(size, dtype=np.float32)
        self.value_buffer = np.zeros(size, dtype=np.short)
        #self.logprobability_buffer = np.zeros(size, dtype=np.float32)
        self.logprobability_buffer = np.zeros(size, dtype=np.short)
...
def lstm(x, sizes, activation=tf.tanh, output_activation=None):
    # Build a feedforward neural network
    for size in sizes[:-1]:
        x = layers.LSTM(units=size, activation=activation, return_sequences=True)(x)
...
env = BSPEnvironmentWrapper(env_name, logname, 0)
#print('env.observation_space.shape', env.observation_space.shape)
observation_dimensions = 1000 #env.observation_space.shape[0]
num_actions = 1000 #env.action_space.n

# Initialize the buffer
buffer = Buffer(observation_dimensions, steps_per_epoch)

# Initialize the actor and the critic as keras models
#observation_input = keras.Input(shape=(1,observation_dimensions), dtype=tf.float32)
observation_input = keras.Input(shape=(observation_dimensions,observation_dimensions), dtype=tf.float32)
#logits = mlp(observation_input, list(hidden_sizes) + [num_actions], tf.tanh, None)
logits = lstm(observation_input, list(hidden_sizes) + [num_actions], tf.tanh, None)
actor = keras.Model(inputs=observation_input, outputs=logits)
value = tf.squeeze(
    #mlp(observation_input, list(hidden_sizes) + [1], tf.tanh, None), axis=1
    lstm(observation_input, list(hidden_sizes) + [1], tf.tanh, None), axis=1
)
...
for epoch in range(epochs):
    ...
    for t in range(steps_per_epoch):
        ...
        observation = observation.reshape(1, observation_dimensions, -1)
        ...
        if terminal or (t == steps_per_epoch - 1):
            ...
            #last_value = 0 if done else critic(observation.reshape(1, 1, -1))
            last_value = 0 if done else critic(observation.reshape(1, observation_dimensions, -1))
