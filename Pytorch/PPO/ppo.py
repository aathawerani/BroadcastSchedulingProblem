...
def ppo(env_fn, actor_critic=core.MLPActorCritic, ac_kwargs=dict(), seed=0, 
    ...
    logger1 = Logger(logname)
    foundbestpath = os.path.join(logpath, bestname + ".log")
    foundmaxpath = os.path.join(logpath, maxname + ".log")

    logger1.printwrite(thread, "Start", env_name)
    logger1.write(params)

    # Instantiate environment
    env = BSPEnvironmentWrapper(env_name, logname, 0) #env_fn()
    #obs_dim = env.observation_space.shape
    #act_dim = env.action_space.shape
    state = np.zeros(params.num_nodes, dtype=np.short)
    obs_dim = env.get_state().shape
    act_dim = env.get_state().shape
    #print("obs_dim", obs_dim)
    #print("act_dim", act_dim)

    # Create actor-critic module
    ac = actor_critic(env.get_state(), env.action_space, **ac_kwargs)
